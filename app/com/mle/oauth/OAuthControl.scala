...
trait OAuthControl extends Controller with Log {
  val creds = Info.googleAuthReader.load
  ...
  def redirURL(implicit req: RequestHeader) = oAuthRedir.absoluteURL(secure = Starter.isHttpsAvailable)
